...
struct statementRecord *newStatement(enum statements st) {
    struct statementRecord *s = (struct statementRecord *)malloc(sizeof(struct statementRecord));

    if(s == 0) { yyerror("out of memory"); }
...
struct statementRecord *genExpressionStatement(struct expressionRecord *e) {
    if(e->resultType != 0) { yyerror("expression statement with nonzero return type"); }
...
struct statementRecord *genAssignmentStatement(struct expressionRecord *left, struct expressionRecord *right) {
    if(left->resultType->ttyp == constantType) { yyerror("cannot assign to a constant value"); }
...
static int canMakeIntoTailCall(struct expressionRecord *e, struct typeRecord *t) {
    // Expression must be function call
    if(e->operator!= doFunctionCall) { return 0; }

    // There must be only one argument
    if(length(e->u.f.args) != 1) { return 0; }

    // There must be only one argument in current context
    if(length(t->u.f.argumentTypes) != 1) { return 0; }

    // The one argument must be the same as current argument
    e = (struct expressionRecord *)e->u.f.args->value;
    if(e->operator!= getOffset) { return 0; }

    if(e->u.o.base->operator!= getCurrentContext) { return 0; }

    if(e->u.o.location != 4) { return 0; }
...
struct statementRecord *genReturnStatement(struct symbolTableRecord *syms, struct expressionRecord *e) {
    struct statementRecord *s = newStatement(returnStatement);

    // Need to check that type matches declared return type
    if(syms->ttype != functionTable) { yyerror("return statement not inside of function"); }

    struct typeRecord *t = syms->definingType->u.f.returnType;
    if(t) {
        // See if a boolean should convert to a relation
        if(typeConformable(relationType, t) && typeConformable(booleanType, e->resultType)) {
            e = relationCheck(syms, e);
        }
        // or vice versa
        else if(typeConformable(booleanType, t) && typeConformable(relationType, e->resultType)) {
            e = booleanCheck(syms, e);
        }

        if(!typeConformable(t, e->resultType)) { yyerror("return type does not match function definition"); }
    } else if(e) {
        yyerror("return expression from within function with no return type");
    }

    s->u.r.e = e;
    if(e && canMakeIntoTailCall(e, syms->definingType)) { s->statementType = tailCall; }

    return s;
}

struct statementRecord *genConditionalStatement(int ln, struct expressionRecord *e, struct statementRecord *tpf,
                                                struct statementRecord *tpl, struct statementRecord *fpf,
                                                struct statementRecord *fpl, struct statementRecord *nt) {
...
struct statementRecord *genWhileStatement(int ln, struct expressionRecord *e, struct statementRecord *stateFirst,
                                          struct statementRecord *stateLast, struct statementRecord *nullState) {
    struct statementRecord *s = genConditionalStatement(ln, e, stateFirst, stateLast, 0, 0, nullState);
...
struct statementRecord *genBody(struct symbolTableRecord *syms, struct statementRecord *code) {
    // Make base for constants
    struct expressionRecord *base = newExpression(getCurrentContext);
    if(syms->ttype == functionTable) { base = genOffset(base, 3, 0, 0); }
    ...
    for(struct list *p = syms->firstSymbol; p; p = p->next) {
        ...
        if(sym->styp == varSymbol && (strcmp(sym->name, "true") == 0 || strcmp(sym->name, "false") == 0)) {
            ...
            /* Guard: initial True/False classes have symbols==NULL until std.led is parsed. */
            struct typeRecord *ct = class_value->resultType;
            struct typeRecord *bct = (ct != NULL && ct->ttyp == classDefType) ? ct->u.q.baseType : NULL;
...
struct expressionRecord *newExpression(enum instructions opcode) {
    if((opcode == getCurrentContext) && (getCC != 0)) { return getCC; }

    struct expressionRecord *e = (struct expressionRecord *)malloc(sizeof(struct expressionRecord));
    if(e == 0) { yyerror("out of memory\n"); }
    e->operator= opcode;
    e->resultType = 0;
    if(opcode == getCurrentContext) { getCC = e; }
...
static struct expressionRecord *genFromSymbol(struct expressionRecord *base, struct symbolRecord *s, int isFunctionTable,
                                              int isGlobals) {
    ...
    switch(s->styp) {
        case varSymbol:
            if(isFunctionTable) {
                e = genOffset(genOffset(base, 3, 0, 0), s->u.v.location, s, s->u.v.typ);
            } else {
                e = genOffset(base, s->u.v.location, s, s->u.v.typ);
        ...
        case classDefSymbol:
            ...
            if(t->ttyp == qualifiedType) {
                ...
                if(t->u.q.baseType->ttyp != classType) { yyerror("confusing case in class instance building"); }
        ...
        case constSymbol:
            if(isFunctionTable) {
                e = genOffset(genOffset(base, 3, 0, 0), s->u.s.location, s, s->u.s.typ);
            } else {
                e = genOffset(base, s->u.s.location, s, s->u.s.typ);
                // if (isGlobals)
                //   e->operator = getGlobalOffset;
            }
            break;

        default: printf("symbol type is %d\n", s->styp); yyerror("Compiler error: unimplemented symbol type");
    }
...
static struct expressionRecord *generateTemporary(struct symbolTableRecord *syms, struct typeRecord *t) {
    char name[100];
...
struct expressionRecord *lookupFunction(struct symbolTableRecord *syms, char *name) {
    struct expressionRecord *e = lookupIdentifier(syms, name);
    if(e->resultType->ttyp != functionType) { yyserror("expecting function for symbol %s", name); }

    return e;
}

static struct expressionRecord *makeMethodIntoFunction(struct typeRecord *ct, char *fieldName) {
    ...
    for(struct list *p = bt->u.c.symbols->u.c.methodTable; p; p = p->next) {
        ...
        if(strcmp(fieldName, s->name) == 0) {
            if(s->styp != functionSymbol) { return 0; }
...
struct expressionRecord *lookupField(struct expressionRecord *base, struct typeRecord *t, char *fieldName) {
    if(base == 0) { yyerror("Compiler error: looking up field in zero base"); }

    if(t == 0) { yyerror("Compiler error: expression with no result type"); }

    if(t->ttyp == constantType) {
        struct expressionRecord *e = lookupField(base, t->u.u.baseType, fieldName);
        if(e) {
    ...
    if(t->ttyp == resolvedType) {
        struct expressionRecord *e = lookupField(base, t->u.r.baseType, fieldName);
        ...
        for(struct list *p = t->u.c.symbols->firstSymbol; p; p = p->next) {
            struct symbolRecord *s = (struct symbolRecord *)p->value;
            if(strcmp(fieldName, s->name) == 0) { return genFromSymbol(base, s, 0, 0); }
        }
        ...
        for(struct list *p = t->u.c.symbols->u.c.methodTable; p; p = p->next) {
            ...
            if(strcmp(fieldName, s->name) == 0) {
                if(s->styp == functionSymbol) {
                    struct expressionRecord *e = newExpression(makeMethodContext);
                    e->u.o.base = base;
...
static struct expressionRecord *lookupAddress(struct symbolTableRecord *syms, char *name, struct expressionRecord *base) {
    switch(syms->ttype) {
        case globals:  // See if it is in the global symbol table
            for(struct list *p = syms->firstSymbol; p; p = p->next) {
                struct symbolRecord *s = (struct symbolRecord *)p->value;
                if(strcmp(name, s->name) == 0) { return genFromSymbol(base, s, 0, 1); }
            }
            return 0;  // Not known
        ...
        case functionTable:
            for(struct list *p = syms->firstSymbol; p; p = p->next) {
                struct symbolRecord *s = (struct symbolRecord *)p->value;
                if(strcmp(name, s->name) == 0) { return genFromSymbol(base, s, 1, 0); }
            }
            // Not local, try next level
            return lookupAddress(syms->surroundingContext, name, genOffset(base, 1, 0, 0));

        case classTable: {
            struct expressionRecord *e = lookupField(base, syms->definingType, name);
            if(e) { return e; }
...
struct expressionRecord *lookupIdentifier(struct symbolTableRecord *syms, char *name) {
    struct expressionRecord *e = lookupAddress(syms, name, newExpression(getCurrentContext));
    if(e == 0) { yyserror("unknown identifier %s", name); }
...
static int argumentsCanMatch(struct typeRecord *t, struct list *args) {
    ...
    if(length(ft->u.f.argumentTypes) != length(q)) { return 0; }
...
struct expressionRecord *generateFunctionCall(struct symbolTableRecord *syms, struct expressionRecord *base, struct list *args,
                                              int isFun) {
    // Make sure base is a function
    struct typeRecord *t = base->resultType;

    if(t->ttyp == constantType) { t = t->u.u.baseType; }
    ...
    if(t->ttyp == resolvedType) {
        ...
        if(ft->ttyp == classDefType) {
            ...
            e->u.n.table = base;
            e->u.n.size = ft->u.q.baseType->u.q.baseType->u.c.symbols->size;
            e->u.n.args = reverse(args);
            e->resultType = fixResolvedType(ft->u.q.baseType, t);
            if(isFun == 0) { yyerror("Value generated by constructor must be used"); }
    ...
    if(!argumentsCanMatch(t, args)) { yyerror("Arguments do not match function declaration"); }
    ...
    if(isFun) {
        if(e->resultType == 0) { yyerror("Using non-value returning function where value expected"); }
    } else {
        if(e->resultType != 0) { yyerror("Using value returning function where no value is expected"); }
    }
...
struct expressionRecord *generateCFunctionCall(char *name, struct list *args, struct typeRecord *rt) {
    struct expressionRecord *e = newExpression(doSpecialCall);

    // See if name is on approved list
    e->u.c.index = -1;
    for(int i = 0; specialFunctionNames[i]; i++) {
        if(strcmp(name, specialFunctionNames[i]) == 0) { e->u.c.index = i; }
    }

    if(e->u.c.index == -1) { yyerror("Unknown cfunction invoked"); }
...
static struct expressionRecord *checkBinarySymbol(struct symbolTableRecord *syms, struct symbolRecord *s, char *name,
                                                  struct expressionRecord *base, struct list *args) {
    if(strcmp(name, s->name) != 0) { return 0; }

    struct expressionRecord *e = genFromSymbol(base, s, 0, 0);

    if(argumentsCanMatch(e->resultType, args)) { return generateFunctionCall(syms, e, args, 1); }

    return 0;
}


static struct expressionRecord *lookupBinaryOperator(struct symbolTableRecord *syms, struct expressionRecord *base, char *name,
                                                     struct list *args) {
    switch(syms->ttype) {
        case globals:
            for(struct list *p = syms->firstSymbol; p; p = p->next) {
                struct expressionRecord *e = checkBinarySymbol(syms, (struct symbolRecord *)p->value, name, base, args);
                if(e != 0) { return e; }
            }
            break;
        ...
        case functionTable:
            ...
            return lookupBinaryOperator(syms->surroundingContext, genOffset(base, 1, 0, 0), name, args);

        case classTable: return lookupBinaryOperator(syms->surroundingContext, genOffset(base, 1, 0, 0), name, args);
    }
...
struct expressionRecord *generateBinaryOperator(struct symbolTableRecord *syms, char *name, struct expressionRecord *left,
                                                struct expressionRecord *right) {
    struct typeRecord *t = left->resultType;
    struct list *args = newList((char *)right, 0);

    if((t->ttyp == classType) || (t->ttyp == resolvedType) || (t->ttyp == constantType) || (t->ttyp == unresolvedType)) {
        // See if it matches a method
        struct expressionRecord *e = lookupField(left, t, name);
        if(e) {
            t = e->resultType;
            if(argumentsCanMatch(t, args)) { return generateFunctionCall(syms, e, args, 1); }
        }
    }

    // Didn't work as method, try to find it as a binary function
    struct expressionRecord *e =
        lookupBinaryOperator(syms, newExpression(getCurrentContext), name, newList((char *)right, newList((char *)left, 0)));

    if(e == 0) { yyserror("cannot find match for binary operator %s", name); }

    return e;
}


struct expressionRecord *generateUnaryOperator(struct symbolTableRecord *syms, char *name, struct expressionRecord *arg) {
    struct typeRecord *t = arg->resultType;
    struct list *args = newList((char *)arg, 0);

    if((t->ttyp == classType) || (t->ttyp == resolvedType) || (t->ttyp == unresolvedType)) {
        // See if it matches a method
        struct expressionRecord *e = lookupField(arg, t, name);
        if(e) {
            t = e->resultType;
            if(argumentsCanMatch(t, 0)) { return generateFunctionCall(syms, e, 0, 1); }
        }
    }

    // Didn't work as method, try to find it as a binary function
    struct expressionRecord *e = lookupBinaryOperator(syms, newExpression(getCurrentContext), name, args);
...
struct expressionRecord *generateLeftArrow(struct symbolTableRecord *syms, struct expressionRecord *left,
                                           struct expressionRecord *right) {
    if(!typeConformable(left->resultType, right->resultType)) { yyerror("Non conformable types used in <-"); }
...
struct expressionRecord *generateForRelation(struct symbolTableRecord *syms, struct expressionRecord *relExp,
    ...
    if(!typeConformable(relationType, relExp->resultType)) { yyerror("for statement must have relation type"); }

    // Make stop condition and statements into a thunk
    if(stopExp == 0) { stopExp = lookupIdentifier(syms, newString("false")); }

    if(!typeConformable(booleanType, stopExp->resultType)) { yyerror("stop condition in for statement must be boolean"); }
...
struct expressionRecord *booleanCheck(struct symbolTableRecord *syms, struct expressionRecord *e) {
...
struct expressionRecord *relationCheck(struct symbolTableRecord *syms, struct expressionRecord *e) {
    // Convert a relation into a boolean, if necessary
...
struct symbolTableRecord *generateFunctionExpression(struct symbolTableRecord *syms, struct list *va, struct typeRecord *rt) {
...
struct statementRecord *generateArithmeticForStatement(int ln, struct symbolTableRecord *syms, struct expressionRecord *target,
    ...
    struct expressionRecord *test = generateBinaryOperator(syms, newString("lessEqual"), target, temp);
    struct statementRecord *is =
        genAssignmentStatement(target, generateBinaryOperator(syms, newString("plus"), target, integerConstant(1)));
...
struct expressionRecord *generateArrayLiteral(struct symbolTableRecord *syms, struct list *exps) {
    // There must be at least one expression
    if(length(exps) < 1) { yyerror("must be at least one expression in array literal"); }
    ...
    s->u.a.form = byValue;
    s->u.a.typ = baseType;
    struct typeRecord *rt = checkQualifications(ae->resultType, newList((char *)s, 0));
    if(rt->ttyp != resolvedType) { yyerror("confusing case in generateArrayLiteral"); }
    if(rt->u.r.baseType->ttyp != classDefType) { yyerror("another confusing case in generateArrayLiteral"); }

    // Make the expression that represents the arguments
    struct expressionRecord *arge = newExpression(doSpecialCall);
    arge->u.c.index = 15;
    arge->u.c.args = newList((char *)integerConstant(length(exps)), reverse(exps));

    // Now build the array
    struct expressionRecord *e = newExpression(buildInstance);
    e->u.n.table = ae;
    e->u.n.size = 4;  // length(exps);
    e->u.n.args = newList((char *)integerConstant(1), newList((char *)integerConstant(length(exps)), newList((char *)arge, 0)));
...
struct expressionRecord *genPatternMatch(struct symbolTableRecord *syms, struct expressionRecord *base,
    ...
    for(; args; args = args->next) {
        ...
        if(f == 0) { yyserror("unknown identifier ", args->value); }
        if(f->operator!= getOffset) { yyerror("variable in pattern must be local\n"); }
        struct expressionRecord *ne = newExpression(makeReference);
